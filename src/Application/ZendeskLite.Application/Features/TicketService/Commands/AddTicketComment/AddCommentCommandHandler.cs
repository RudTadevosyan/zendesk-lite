...
public class AddCommentCommandHandler : IRequestHandler<AddCommentCommand, Result>
{
    ...
    public async Task<Result> Handle(AddCommentCommand request, CancellationToken ct)
    {
        ...
        _logger.LogInformation("Attempting to add comment to TicketId: {TicketId} by User: {UserId}", request.TicketId, request.UserId);
        ...
        if (ticket.CustomerId != request.UserId && !request.IsAdminOrAgent)
        {
            _logger.LogWarning("Unauthorized comment attempt on Ticket {TicketId} by User {UserId}", request.TicketId, request.UserId);
        ...
        try
        {
            ...
            _logger.LogInformation("Successfully added comment to Ticket {TicketId} by User {UserId}", ticket.Id, request.UserId);

            return Result.Success();
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "An error occurred while adding a comment to Ticket {TicketId} by User {UserId}", request.TicketId, request.UserId);
